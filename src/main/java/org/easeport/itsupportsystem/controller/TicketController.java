...
public class TicketController {
    ...
    public ResponseEntity<?> getInit() {
        try {
            ticketService.initWs();

            return ResponseEntity.ok()
                    .body(new MessageResponse("Init success"));
    ...
    public ResponseEntity<?> assignTicket(@PathVariable("ticketId") Long ticketId, Authentication authentication) {
        ...
        } catch (TicketNotFoundException | UserNotFoundException | JsonProcessingException e) {
            return ResponseEntity.badRequest()
                    .body(new MessageResponse(e.getMessage()));
        }
    }
        ...
        try {
            ...
            if(sent) {
                return ResponseEntity.ok()
                 ...
                 return ResponseEntity.badRequest()
                        .body(new MessageResponse("Answer couldn't be sentee"));
            }
    ...
    @PostMapping("/sendAnswer/{ticketId}")
    @PreAuthorize("hasRole('USER')")
    public ResponseEntity<?> sendAnswer(@PathVariable("ticketId") Long ticketId, Authentication authentication) {
        try {
            UserPrincipal userPrincipal = (UserPrincipal) authentication.getPrincipal();
            User user = userService.findByUsername(userPrincipal.getUsername());

            boolean sent = ticketService.sendAnswer(user, ticketId);
            if(sent) {
                return ResponseEntity.ok()
                        .body(new MessageResponse("Answer was sent!"));
            } else {
